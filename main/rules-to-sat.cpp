...
void read_rif_rules(deque<RIFRule> &rules, deque<RIFCondition> &repls, deque<RIFCondition> &norepls, deque<RIFCondition> &arbs, deque<RIFCondition> &noarbs) {
  map<string, string> defns;
  string line;
  while (!cin.eof()) {
    getline(cin, line);
    if (line.size() >= 8 && line.substr(0, 8) == string("#DEFINE ")) {
      size_t space = line.find_first_of(" \t", 8);
      if (space == string::npos) {
        cerr << "[WARNING] Ignoring malformed: " << line << endl;
        continue;
      }
      size_t nonspace = line.find_first_not_of(" \t", space+1);
      if (nonspace == string::npos) {
        nonspace = line.size();
      }
      defns[line.substr(8, space-8)] = line.substr(nonspace);
      continue;
    }
    ...
    while (bracket != string::npos) {
      ...
      if (close == string::npos) {
        cerr << "[WARNING] No closing brace in: " << line << endl;
        break;
      }
      string label = line.substr(bracket+1, close-bracket-1);
      if (defns.count(label) <= 0) {
        cerr << "[WARNING] Ignoring undefined {" << label << "} in: " << line << endl;
        bracket = line.find('{', bracket+1);
      } else {
        string replaced = line.substr(0, bracket);
        replaced.append(defns[label]);
        replaced.append(line.substr(close+1));
        line.swap(replaced);
        bracket = line.find('{');
      }
    }
...
private:
  ...
  Rule rule;
public:
  Var(enum Pred pred, const Pattern &patt)
      : pred(pred) {
...
void make_literal(const bool sign, enum Pred pred, const Pattern &patt) {
  Var var(pred, patt);
  make_literal(sign, var);
}
...
void make_literal(int abandon, const Rule &rule) {
  make_literal(abandon == ABANDON, rule);
}
...
void make_implication(const bool sign1, int repl1, const Pattern &patt1,
                      const bool sign2, int repl2, const Pattern &patt2) {
  make_literal(!sign1, repl1 == REPLICATE, patt1);
  make_literal(sign2, repl2 == REPLICATE, patt2);
  terminate_clause();
}

void make_implication(const bool sign1, int repl, const Pattern &patt,
                      const bool sign2, int abandon, const Rule &rule) {
  make_literal(!sign1, repl == REPLICATE, patt);
  make_literal(sign2 ^ (abandon == ABANDON), rule);
  terminate_clause();
}

bool subset_of_one_of(const Pattern &p, const set<Pattern> &ps) {
//  // HACK
//  RIFAtomic atom = p.getAtom();
//  if (atom.getType() == EXTERNAL) {
//    return true;
//  }
//  if (atom.getType() == EQUALITY) {
//    pair<RIFTerm, RIFTerm> eq = atom.getEquality();
//    if (eq.first.isGround() && eq.second.isGround()) {
//      return true;
//    }
//  }
//  // HACK
...
bool all_but_n_subset_of_one_of(size_t n, const multiset<Pattern> &ps1, const set<Pattern> &ps2) {
  size_t i = 0;
  multiset<Pattern>::const_iterator it;
  for (it = ps1.begin(); it != ps1.end(); ++it) {
    if (!subset_of_one_of(*it, ps2)) {
      if (++i > n) {
        return false;
      }
      continue;
...
bool exists_subset_of_one_of(const multiset<Pattern> &ps1, const set<Pattern> &ps2) {
  size_t i = 0;
  multiset<Pattern>::const_iterator it;
  for (it = ps1.begin(); it != ps1.end(); ++it) {
    if (subset_of_one_of(*it, ps2)) {
...
void sacrifice_rules(set<Rule> &rules, const set<Pattern> &repls, const set<Pattern> &norepls,
                     const set<Pattern> &arbs, const set<Pattern> &noarbs) {
  // Assuming arbs and repls are closed.  That is, for any pattern in a rule
  // in rules, if it is not in arbs, it is not arbitrary, and if it is not
  // in repls, it is not replicated.
  set<Rule> possible;
  set<Rule>::iterator rit;
  for (rit = rules.begin(); rit != rules.end(); ++rit) {
    const multiset<Pattern> &posc = rit->getPositiveConditions();
    const multiset<Pattern> &negc = rit->getNegativeConditions();
    const multiset<Pattern> &act = rit->getActions();

    if (CLOSED && !all_but_n_subset_of_one_of(0, negc, repls)) {
      make_literal(ABANDON, *rit);
      terminate_clause();
    } else if (CLOSED && !rit->isAssertive() && !all_but_n_subset_of_one_of(0, act, repls)) {
      make_literal(ABANDON, *rit);
      terminate_clause();
    } else if (all_but_n_subset_of_one_of(0, posc, repls)) {
      make_literal(-ABANDON, *rit);
      terminate_clause();
      possible.insert(*rit);
    } else if (rit->isAssertive() &&
               all_but_n_subset_of_one_of(1, posc, repls) &&
               all_but_n_subset_of_one_of(0, act, arbs)) {
      make_literal(-ABANDON, *rit);
      terminate_clause();
      possible.insert(*rit);
    } else if (CLOSED) {
  ...
  for (rit = rules.begin(); rit != rules.end(); ++rit) {
    ...
    const multiset<Pattern> &negc = rit->getNegativeConditions();
    const multiset<Pattern> &act = rit->getActions();
    if (!all_but_n_subset_of_one_of(0, negc, repls)) {
      make_literal(ABANDON, *rit);
      terminate_clause();
    } else if (!rit->isAssertive() && !all_but_n_subset_of_one_of(0, act, repls)) {
      make_literal(ABANDON, *rit);
      terminate_clause();
    } else if (all_but_n_subset_of_one_of(0, posc, repls)) {
      make_literal(-ABANDON, *rit);
...
void build_cnf(set<Rule> &rules, const set<Pattern> &repls, set<Pattern> &norepls, const set<Pattern> &arbs, set<Pattern> &noarbs) {
  set<Pattern> patterns;
  ...
  if (PRAGMAS) {
    ...
    patterns.insert(repls.begin(), repls.end());
  ...
  for (rit = rules.begin(); rit != rules.end(); ++rit) {
    ...
    patterns.insert(posc.begin(), posc.end());
    patterns.insert(negc.begin(), negc.end());
    patterns.insert(act.begin(), act.end());

    multiset<Pattern>::const_iterator it;
    for (it = posc.begin(); it != posc.end(); ++it) {
      multiset<Pattern>::const_iterator it2;
      for (it2 = posc.begin(); it2 != posc.end(); ++it2) {
        if (it != it2) {
          if (SACRIFICE) make_literal(ABANDON, *rit);
          make_literal(true, REPLICATE, *it);
          make_literal(true, REPLICATE, *it2);
          terminate_clause();
        }
      }
    }

    for (it = negc.begin(); it != negc.end(); ++it) {
      if (SACRIFICE) make_literal(ABANDON, *rit);
      make_literal(true, REPLICATE, *it);
      terminate_clause();
    }
    ...
    if (rit->isAssertive()) {
      for (it = act.begin(); it != act.end(); ++it) {
        ...
        for (it2 = posc.begin(); it2 != posc.end(); ++it2) {
          if (SACRIFICE) make_literal(ABANDON, *rit);
          make_literal(true, ARBITRARY, *it);
          make_literal(true, REPLICATE, *it2);
          terminate_clause();
        }
      }
    } else {
      if (act.begin() != act.end()) {
        for (it = posc.begin(); it != posc.end(); ++it) {
          if (SACRIFICE) make_literal(ABANDON, *rit);
          make_literal(true, REPLICATE, *it);
          terminate_clause();
  ...
  set<Pattern>::iterator pit;
  if (EAGER) {
    for (pit = patterns.begin(); pit != patterns.end(); ++pit) {
      set<Pattern>::iterator pit2 = pit;
      for (++pit2; pit2 != patterns.end(); ++pit2) {
        if (pit->overlapsWith(*pit2)) {
          // This is very inexact, but it will be fast, and that is why it
          // is made available via the --eager option.  Satisfaction of the
          // formula in the this case is a sufficient, but not necessary,
          // condition.  In other words, a solution using --eager is also
          // a solution without using --eager, but there may be more
          // (and probably better) solutions that can be found without using
          // --eager.
          make_implication(false, ARBITRARY, *pit, false, ARBITRARY, *pit2);
          make_implication(false, REPLICATE, *pit, false, REPLICATE, *pit2);
          make_implication(false, ARBITRARY, *pit2, false, ARBITRARY, *pit);
          make_implication(false, REPLICATE, *pit2, false, REPLICATE, *pit);
        }
      }
    }
  } else {
    int delta = patterns.size();
    while (delta > 0) {
      set<Pattern> intersections;
      for (pit = patterns.begin(); pit != patterns.end(); ++pit) {
        set<Pattern>::iterator pit2 = pit;
        for (++pit2; pit2 != patterns.end(); ++pit2) {
          Pattern inter = pit->intersectWith(*pit2);
          if (!inter.empty()) {
      ...
      delta = -patterns.size();
      patterns.insert(intersections.begin(), intersections.end());
      delta += patterns.size();
    }
  }
  for (pit = patterns.begin(); pit != patterns.end(); ++pit) {
    set<Pattern>::iterator pit2 = pit;
    make_implication(true, REPLICATE, *pit, false, ARBITRARY, *pit);
    for (++pit2; pit2 != patterns.end(); ++pit2) {
      if (pit->subsetOf(*pit2)) {
        make_literal(false, REPLICATE, *pit2);
        make_literal(true, REPLICATE, *pit);
        terminate_clause();
        make_literal(false, ARBITRARY, *pit2);
        make_literal(true, ARBITRARY, *pit);
        terminate_clause();
      }
      if (pit2->subsetOf(*pit)) {
        make_literal(false, REPLICATE, *pit);
        make_literal(true, REPLICATE, *pit2);
        terminate_clause();
        make_literal(false, ARBITRARY, *pit);
        make_literal(true, ARBITRARY, *pit2);
        terminate_clause();
      }
    }
    if (CLOSED) {
      if (!subset_of_one_of(*pit, repls)) {
        norepls.insert(*pit);
      }
      if (!subset_of_one_of(*pit, arbs)) {
        noarbs.insert(*pit);
      }
    }
  }
  if (PRAGMAS) {
    for (pit = repls.begin(); pit != repls.end(); ++pit) {
      make_literal(true, REPLICATE, *pit);
      terminate_clause();
    }
    for (pit = arbs.begin(); pit != arbs.end(); ++pit) {
      make_literal(true, ARBITRARY, *pit);
      terminate_clause();
    }
    for (pit = norepls.begin(); pit != norepls.end(); ++pit) {
      make_literal(false, REPLICATE, *pit);
      terminate_clause();
    }
    for (pit = noarbs.begin(); pit !=  noarbs.end(); ++pit) {
      make_literal(false, ARBITRARY, *pit);
      terminate_clause();
    }
  }
...
void parse_args(int argc, char **argv) {
  ...
  for (i = 1; i < argc; ++i) {
    if (strcmp(argv[i], "--sacrifice") == 0 ||
      ...
      CLOSED = true;
      PRAGMAS = true;
    } else if (strcmp(argv[i], "--pragmas") == 0 ||
               strcmp(argv[i], "-p") == 0) {
      PRAGMAS = true;
...
int doit(int argc, char **argv) {
  ...
  build_cnf(rules, repl_patterns, no_repl_patterns, arb_patterns, no_arb_patterns);
  print_cnf();
  cnfdata.clear();
  num2var.clear();
  var2num.clear();
